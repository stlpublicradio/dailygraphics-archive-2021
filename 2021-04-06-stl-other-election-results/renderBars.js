...
var renderBarChart = function(config) {
  // Setup
  var { labelColumn, valueColumn, winnerColumn } = config;
  ...
  votes = config.data.map(d => d[valueColumn])
  const vote_total = votes.reduce((a,b) => a+b, 0);
  ...
  chartElement
    ...
    .attr("class", function(d, i) {
      if (d[winnerColumn] == 'Y') 
      {return `winner bar-${i} ${classify(d[labelColumn])}`}
      else {return `bar-${i} ${classify(d[labelColumn])}`}});
...
var fmtPct = d3.format('.1%')
  ...
  chartElement
    ...
    .text(function(d) 
    {
      if (config.container == "#bar-chart-school-bd")
      {
        return fmtComma(d[valueColumn])
      }
      else {
        return fmtComma(d[valueColumn]) + " (" + fmtPct(d[valueColumn]/vote_total) + ")"
      }
    }
      )
